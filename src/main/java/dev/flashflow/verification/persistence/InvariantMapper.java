            ...
            SELECT
              (SELECT COUNT(*) FROM activity_sku_stock
               WHERE available_stock < 0 OR reserved_stock < 0 OR sold_stock < 0
                  OR initial_stock <> available_stock + reserved_stock + sold_stock)
                AS negative_or_unbalanced_stocks,
              (SELECT COUNT(*) FROM orders o LEFT JOIN purchase_claim c ON c.order_id = o.id
               WHERE o.status IN ('PENDING_PAYMENT', 'PAID') AND c.order_id IS NULL)
                AS effective_orders_without_claims,
              (SELECT COUNT(*) FROM purchase_claim c JOIN orders o ON o.id = c.order_id
               WHERE o.status NOT IN ('PENDING_PAYMENT', 'PAID'))
                AS claims_without_effective_orders,
